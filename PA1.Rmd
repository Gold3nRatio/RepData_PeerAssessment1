...
To observe the user's daily activty base on hours of the day, the following chart will uses the average steps taken on a given time interval over 2-monthof data to plot a time series chart.  
...
## Missing Values ##
There are many missing values within the database.  To replace the missing values, the average steps for each interval is calcuated over the 2-month period and assigned if steps are missing. 
...
The following R code tests each row and assigns "Weekend" if the date is either Saturday or Sunday.  The rest of the dates will get a "Weekday" value.  Using the new dataset, the time interval average is calcuated between the weekday and weekend categories.
```{r}
